...
import com.opendroid.ai.data.models.withResolvedActiveModel
...
class SettingsViewModel @Inject constructor(
    ...
    init {
        ...
        viewModelScope.launch {
            settingsRepository.llmConfig.collect { config ->
                if (!isLoaded) {
                    _llmConfig.value = config.withResolvedActiveModel()
                    isLoaded = true
                } else {
                    val current = _llmConfig.value
                    val merged = config.copy(
                        ...
                        copilotUrl = current.copilotUrl,
                        // The model box is edited in place and written after a
                        // pause, so a snapshot arriving mid-edit still carries
                        // the previous string. Keep what is on screen unless
                        // the provider itself moved underneath it.
                        activeModel = if (config.activeProvider == current.activeProvider) {
                            current.activeModel
                        } else {
                            config.activeModel
                        }
                    )
                    _llmConfig.value = if (config.activeProvider == current.activeProvider) {
                        merged
                    } else {
                        merged.withResolvedActiveModel()
                    }
                }
            }
        }
    ...
    // ── Codex sign-in ────────────────────────────────────────────────────────
    //
    // Nothing is typed in here. The phone runs the OAuth flow itself, keeps the
    // resulting session in the Keystore, and that session is the credential -
    // see core/llm/codex/. All this view model does is start the flow and
    // surface what the manager already knows.

    val codexAuth: StateFlow<CodexAccountState> = codexAuthManager.state

    fun refreshCodexState() = codexAuthManager.refreshState()

    fun startCodexSignIn() {
        viewModelScope.launch { codexAuthManager.signIn() }
    }

    fun signOutOfCodex() = codexAuthManager.signOut()
