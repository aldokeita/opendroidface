...
object ProviderCatalog {
    ...
    val providers: List<ProviderSpec> = listOf(
        ...
        // The ChatGPT account signed in on this phone, talking to the Codex
        // backend directly. No key and no endpoint to configure - the session
        // is the credential.
        ProviderSpec("Codex", "gpt-5-codex"),
    ...
    fun requiresApiKey(providerName: String): Boolean = when (canonicalName(providerName)) {
        ...
        "Cohere",
        "DeepSeek",
        // Codex is absent on purpose: it authenticates with an OAuth session
        // held in the Keystore, so there is no key for Settings to ask for.
